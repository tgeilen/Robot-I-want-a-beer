...
Lastly, further relevant projects enabled by the "Beer Robot" are discussed.
...
To ensure an equal amount of liquid in each drink, independent of whether it is the first or secound pour from a bottle, the robot measures the bottle's weight before starting to pour the drink. The pouring time is then adjusted based on the bottle's weight.
